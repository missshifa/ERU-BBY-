...
module.exports.onStart = async ({ api, event, args }) => {
  ...
  const prefix = (global.GoatBot?.config?.prefix) || "!"; // fallback prefix

  const styles = {
    bold: (text) => text.split("").map(c => String.fromCodePoint(c.charCodeAt(0) + 0x1D5D4 - 0x41)).join(""),
...
│
│ 💡 ${styles.italic("Use")} "${prefix}tempmail --inbox ${email}" 
...
╰───────────────────────────────────╯`;
      ...
      api.sendMessage(`❌ Error generating email:\n${error}`, event.threadID, event.messageID);
    }
  }
...
╰───────────────────────────────────╯`;
      ...
      api.sendMessage(`❌ Error checking inbox:\n${error}`, event.threadID, event.messageID);
    }
  }
...
╭─────『 📋 TempMail Help 』─────╮
│
│ ${prefix}tempmail --gen [name]
│ ┗━━➤ Generate a temporary email
│
│ ${prefix}tempmail --inbox [email]
│ ┗━━➤ Check email inbox
│
╰───────────────────────────────────╯`;
